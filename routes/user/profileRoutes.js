...
router.get(
  ...
  (req, res) => {
    res.render("user/verify-emailpass-otp", {
      newEmail: req.session.newEmail || "",

      flow: req.query.flow || "email",
    });
  },
);
...
router.get("/addresses", isLoggedIn, userBlockCheckMiddleware, loadAddresses);

/* =========================
   ADD ADDRESS
========================= */

router.post("/add-address", isLoggedIn, userBlockCheckMiddleware, addAddress);
