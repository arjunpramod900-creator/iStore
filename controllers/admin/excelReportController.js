...
export const downloadSalesReportExcel = async (req, res) => {
  try {
    ...
    worksheet.addRow([
      `Report Period: ${report.resolvedStartDate} - ${report.resolvedEndDate}`,
    ]);
        ...
        order.items.forEach((item) => {
          ...
          const styleStatusCell = (cell, status) => {
            ...
            if (statusColors[statusUpper]) {
              cell.font = {
                color: { argb: statusColors[statusUpper] },
                bold: true,
              };
            }
          };
