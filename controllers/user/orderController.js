...
export const loadOrdersPage = async (req, res) => {
  try {
    ...
    const response = await loadOrdersService({
      userId,
      search,
      status,
      sort,
...
export const cancelOrderItem = async (req, res) => {
  try {
    ...
    const response = await cancelOrderItemService(
      userId,
      orderId,
      itemId,
      reason,
    ...
    return res
      .status(500)
      .json({ success: false, message: "Item cancellation failed" });
  }
};
...
export const returnOrder = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Return request failed" });
  }
};
...
export const returnOrderItem = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Item return request failed" });
  }
};
...
export const downloadInvoice = async (req, res) => {
  try {
    ...
    const offerDiscount =
      ...
      order.items.reduce((s, i) => s + (i.offerDiscount || 0) * i.quantity, 0);
    const subtotal =
      order.pricingSnapshot?.originalSubtotal ??
      order.items.reduce((s, i) => s + i.price * i.quantity, 0);
    const mrpTotal = order.pricingSnapshot?.originalSubtotal
    ...
    const couponDiscount =
      ...
      order.items.reduce((s, i) => s + (i.couponDiscount || 0), 0);

    const summaryTax =
      order.pricingSnapshot?.originalTaxAmount ?? order.taxAmount;
    const summaryDelivery =
      order.pricingSnapshot?.originalDeliveryCharge ?? order.deliveryCharge;
    const summaryTotal =
      order.pricingSnapshot?.originalFinalAmount ?? order.finalAmount;
    ...
    const isRevised =
      order.finalAmount <
        (order.pricingSnapshot?.originalFinalAmount ||
          order.finalAmount + refundAmount) && hasActiveItems;

    let summaryCardsH = 195;
    if (refundAmount > 0 || isRevised) {
      summaryCardsH = hasActiveItems
        ? 385
        : 320; /* taller if revised summary exists */
    }
    const footerH = 80;
    ...
    doc
      ...
      .fillColor("#603763")
      .font("Helvetica-Bold")
      .text(`Invoice #${order.orderId}`, 650, 82);
    ...
    doc
      ...
      .fontSize(13)
      .font("Helvetica-Bold")
      .text("Order Details", 455, startY + 15);
    doc
      .fillColor("#1A1C1D")
      .fontSize(11)
      .font("Helvetica")
    ...
    doc
      .roundedRect(455, startY + 76, 110, 20, 8)
      .fillAndStroke(orderStyle.bg, orderStyle.bg);
    doc
      .fillColor(orderStyle.text)
      .fontSize(10)
      .font("Helvetica-Bold")
      .text(orderStyle.label, 460, startY + 82, {
        width: 100,
        align: "center",
      });
    ...
    const shippingY = 270;
    doc
      .roundedRect(50, shippingY, 742, 130, 12)
    ...
    doc
      .moveTo(420, shippingY + 30)
      .lineTo(420, shippingY + 110)
      .strokeColor("#E6D7EC")
      .stroke();
    doc
    ...
    doc
      .rect(50, tableTop, 742, order.items.length * itemRowH + 20)
      .strokeColor("#E6D7EC")
      .stroke();
    ...
    order.items.forEach((item, index) => {
      const itemStyle = getOrderStatusStyle(
      ...
      /* Item status badge */
      doc
        .roundedRect(248, y - 2, 80, 17, 6)
        .fillAndStroke(itemStyle.bg, itemStyle.bg);
      doc
        .fillColor(itemStyle.text)
        .fontSize(8)
        .font("Helvetica-Bold")
    ...
    doc
      .roundedRect(50, summaryY, 350, 170, 12)
      .fillAndStroke("#FBF8FC", "#E6D7EC");
    doc
      .fillColor("#603763")
      .fontSize(13)
    ...
    doc
      .roundedRect(155, summaryY + 70, 80, 17, 6)
      .fillAndStroke(payStyle.bg, payStyle.bg);
    doc
      .fillColor(payStyle.text)
      .fontSize(9)
      .font("Helvetica-Bold")
      .text(order.paymentStatus, 157, summaryY + 76, {
        width: 76,
        align: "center",
      });
    ...
    doc
      .roundedRect(155, summaryY + 97, 100, 17, 6)
      .fillAndStroke(orderStyle.bg, orderStyle.bg);
    doc
      .fillColor(orderStyle.text)
      .fontSize(9)
      .font("Helvetica-Bold")
      .text(orderStyle.label, 157, summaryY + 103, {
        width: 96,
        align: "center",
      });
    ...
    const originalFinalAmountForInvoice =
      order.pricingSnapshot?.originalFinalAmount ||
      order.finalAmount + refundAmount;
    /* Payment Summary card */
    doc
      .roundedRect(440, summaryY, 350, 170, 12)
      .fillAndStroke("#FBF8FC", "#E6D7EC");
    doc
      .fillColor("#603763")
      .fontSize(13)
      .font("Helvetica-Bold")
      .text(
        isRevised ? "Original Order Summary" : "Payment Summary",
        455,
        summaryY + 15,
      );
    ...
    if (isRevised) {
      doc
        ...
        .text("(Outdated — See Revised Summary)", 620, summaryY + 18, {
          width: 150,
          align: "right",
        });
    }
    ...
    summaryRows.push({
      label: "MRP Total",
      value: `Rs.${mrpTotal.toLocaleString("en-IN")}`,
    });

    if (offerDiscount > 0) {
      summaryRows.push({
        label: "Offer Discount",
        value: `- Rs.${offerDiscount.toLocaleString("en-IN")}`,
      });
    }

    summaryRows.push({
      label: "Subtotal",
      value: `Rs.${subtotal.toLocaleString("en-IN")}`,
    });

    if (couponDiscount > 0) {
      summaryRows.push({
        label: `Coupon ${order.couponCode ? "(" + order.couponCode + ")" : ""}`,
        value: `- Rs.${couponDiscount.toLocaleString("en-IN")}`,
      });
    }

    summaryRows.push({
      label: "Tax",
      value: `Rs.${summaryTax.toLocaleString("en-IN")}`,
    });
    summaryRows.push({
      label: "Shipping",
      value:
        summaryDelivery === 0
          ? "Free"
          : `Rs.${summaryDelivery.toLocaleString("en-IN")}`,
    });

    summaryRows.forEach((row, i) => {
      const rowY = summaryY + 44 + i * 16;
      doc
        .fillColor(isRevised ? "#9CA3AF" : "#7A7A7A")
        .fontSize(9)
        .font("Helvetica")
        .text(row.label, 455, rowY);
      doc
        .fillColor(isRevised ? "#9CA3AF" : "#1A1C1D")
        .fontSize(9)
        .font(isRevised ? "Helvetica-Oblique" : "Helvetica")
        .text(row.value, 710, rowY, { align: "right", width: 60 });
    });

    /* Divider + Total */
    doc
      .moveTo(455, summaryY + 142)
      .lineTo(775, summaryY + 142)
      .strokeColor("#E6D7EC")
      .stroke();
    doc
      .fillColor(isRevised ? "#9CA3AF" : "#603763")
      .fontSize(13)
      .font("Helvetica-Bold")
      .text("Total", 455, summaryY + 150)
      .text(`Rs.${summaryTotal.toLocaleString("en-IN")}`, 690, summaryY + 150, {
        align: "right",
        width: 80,
      });
    ...
    if (refundAmount > 0) {
      doc
        .roundedRect(440, refundY, 350, 120, 12)
      ...
      const refRows = [
        {
          label: "Original Order Paid",
          value: `Rs.${originalFinalAmountForInvoice.toLocaleString("en-IN")}`,
        },
        {
          label: "Revised Order Value",
          value: `- Rs.${order.finalAmount.toLocaleString("en-IN")}`,
        },
      ];
      ...
      if (mathDiff > refundAmount) {
        refRows.push({
          ...
          value: `+ Rs.${(mathDiff - refundAmount).toLocaleString("en-IN")}`,
        });
      }
      ...
      doc
        .moveTo(455, refundY + 92)
        .lineTo(775, refundY + 92)
        .strokeColor("#D1D5DB")
        .stroke();
      doc
        .fillColor("#111827")
        ...
        .text(
          `Rs.${refundAmount.toLocaleString("en-IN")}`,
          690,
          refundY + 100,
    ...
    if (isRevised) {
      ...
      if (activeItems.length > 0) {
        const revMRP = activeItems.reduce(
          (s, i) => s + (i.originalPrice || i.price) * i.quantity,
          0,
        );
        const revOffer = activeItems.reduce(
          (s, i) => s + (i.offerDiscount || 0) * i.quantity,
          0,
        );
        const revSubtotal = activeItems.reduce(
          (s, i) => s + i.price * i.quantity,
          0,
        );
        const revCoupon = activeItems.reduce(
        ...
        doc
          .roundedRect(50, refundY, 350, 170, 12)
          .fillAndStroke("#FBF8FC", "#E6D7EC");
        doc
          .fillColor("#603763")
          .fontSize(13)
        ...
        revRows.push({
          label: "MRP Total",
          value: `Rs.${revMRP.toLocaleString("en-IN")}`,
        });
        if (revOffer > 0)
          revRows.push({
            label: "Offer Discount",
            value: `- Rs.${revOffer.toLocaleString("en-IN")}`,
          });
        revRows.push({
          label: "Subtotal",
          value: `Rs.${revSubtotal.toLocaleString("en-IN")}`,
        });
        if (revCoupon > 0) {
          revRows.push({
            label: `Coupon`,
            value: `- Rs.${revCoupon.toLocaleString("en-IN")}`,
          });
        ...
        revRows.push({
          ...
          value:
            order.deliveryCharge === 0
              ? "Free"
              : `Rs.${order.deliveryCharge.toLocaleString("en-IN")}`,
        });
        ...
        revRows.forEach((row, i) => {
          ...
          doc
            ...
            .fontSize(9)
            .font("Helvetica")
            .text(row.label, 65, rowY);
          doc
            .fillColor("#1A1C1D")
            .fontSize(9)
            .font("Helvetica")
            .text(row.value, 320, rowY, { align: "right", width: 60 });
        });

        doc
          .moveTo(65, refundY + 142)
          .lineTo(385, refundY + 142)
          .strokeColor("#E6D7EC")
          .stroke();
        doc
          .fillColor("#603763")
    ...
    doc
      .fontSize(9)
      .fillColor("#A0A0A0")
      .text("Order generated electronically by iStore", 0, footerY + 32, {
        align: "center",
        width: 842,
      });
