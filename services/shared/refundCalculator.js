...
*/
  const originalSubtotal =
    order.pricingSnapshot?.originalSubtotal ?? order.subtotal ?? itemBase;

  const itemRatio = originalSubtotal > 0 ? itemBase / originalSubtotal : 0;

  /* Original values from pricing snapshot */
  const originalCoupon = order.pricingSnapshot?.originalCouponDiscount ?? 0;

  const originalTax = order.pricingSnapshot?.originalTaxAmount ?? 0;

  const originalDelivery = order.pricingSnapshot?.originalDeliveryCharge ?? 0;

  const proportionalCoupon = Math.round(originalCoupon * itemRatio);

  const proportionalTax = Math.round(originalTax * itemRatio);
  ...
  const deliveryRefund = activeItemsAfter.length === 0 ? originalDelivery : 0;

  const refundAmount = Math.floor(
    itemBase - proportionalCoupon + proportionalTax + deliveryRefund,
  );
...
export const calculateFullOrderRefund = (order) => {
  ...
  return {
    ...
    breakdown: {
      subtotal: order.pricingSnapshot?.originalSubtotal ?? order.subtotal,

      offerDiscount:
        order.pricingSnapshot?.originalOfferDiscount ?? order.offerDiscount,

      couponDiscount:
        order.pricingSnapshot?.originalCouponDiscount ?? order.couponDiscount,

      taxAmount: order.pricingSnapshot?.originalTaxAmount ?? order.taxAmount,
