...
export const loadInventoryService = async (query) => {
  ...
  variants.forEach((variant) => {
    variant.inventoryValue = variant.stock * variant.price;
  });
  ...
  const totalUnits = allVariants.reduce(
    (acc, item) => acc + item.stock,

    0,
  );

  const inventoryValue = allVariants.reduce(
  ...
  const outOfStockCount = await Variant.countDocuments({
    isDeleted: false,
